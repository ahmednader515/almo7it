...
export class SessionManager {
  ...
  // Validate session - supports multi-device for ADMIN/ADMIN_ASSISTANT
  static async validateSession(
    ...
    // For ADMIN/ADMIN_ASSISTANT: validate by userId + isActive (multi-device)
    if (userId) {
      ...
      if (user && (user.role === "ADMIN" || user.role === "ADMIN_ASSISTANT")) {
        return {
